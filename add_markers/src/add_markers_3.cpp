...
double tolerance = 0.1; // distance from robot to marker to be considered hit
...
void OdomCheck(const nav_msgs::Odometry &odom_msg){
  ...
  if (distance(position_x, pickup_zone_x, position_y, pickup_zone_y) < tolerance){
    near_pickup_zone = true;
  }
  if (distance(position_x, drop_off_zone_x, position_y, drop_off_zone_y) < tolerance){
