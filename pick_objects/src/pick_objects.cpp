...
int main(int argc, char** argv){
  ...
  goal.target_pose.pose.position.x = pickup_zone_x; // from "zones.h"
  goal.target_pose.pose.position.y = pickup_zone_y; // from "zones.h"
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
  {
    ...
    goal.target_pose.pose.position.x = drop_off_zone_x; // from "zones.h"
    goal.target_pose.pose.position.y = drop_off_zone_y; // from "zones.h"
    goal.target_pose.pose.orientation.w = 1.0; // w=0 means quaternio; w=1 means Euler
